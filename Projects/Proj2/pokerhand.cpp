...
int PokerHand::cmp(PokerHand &otherHand) {
	if (m_rank < otherHand.getRank()) {
		...
		switch(m_rank) {
			...
			case StraightFlush:
				if(otherHand.m_cards[FIVE - 1].points() < m_cards[FIVE - 1].points()) {
					return 1;
				} else if(otherHand.m_cards[FIVE - 1].points() > m_cards[FIVE - 1].points()) {
					return -1;
				} else {
					return 0;
				}
			...
			case Flush:
				for(int i = FIVE - 1; i >= 0; i--) {
					if(otherHand.m_cards[i].points() < m_cards[i].points()) {
						return 1;
					} else if(otherHand.m_cards[i].points() > m_cards[i].points()) {
						return -1;
					}
				}
